...
import {
  ...
  fetchGuardianPendingRequests,
  rejectGuardianRequest,
} from "@/lib/api/guardian"
import type { GuardianPendingRequest } from "@/lib/api/types"

interface PendingWardRequestsProps {
  refreshKey?: number
  onChanged?: () => void
}
...
function requestLabel(request: GuardianPendingRequest) {
  return `${request.requester_name} wants to link with you`
}

export default function PendingWardRequests({
  refreshKey = 0,
  onChanged,
}: PendingWardRequestsProps) {
  const [requests, setRequests] = useState<GuardianPendingRequest[]>([])
  ...
  const loadRequests = useCallback(async () => {
    setError(null)
    try {
      const result = await fetchGuardianPendingRequests()
  ...
  return (
    <section className="rounded-lg border border-[#222] bg-[#111] p-5">
      ...
      {!loading && requests.length === 0 && (
        <p className="text-sm text-[#666]">No pending ward requests.</p>
      )}
          ...
          return (
            ...
            >
              ...
              <p className="mt-1 text-xs text-[#666]">
                Accept to monitor their SOS alerts after linking.
              </p>
