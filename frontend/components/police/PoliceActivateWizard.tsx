...
import {
  setPoliceActivationPassword,
  verifyPoliceActivationOtp,
} from "@/lib/api/auth"
import { isApiError } from "@/lib/api/client"
import {
  clearPoliceActivationSession,
  loadPoliceActivationSession,
  type PoliceActivationSession,
} from "@/lib/auth/police-activation-session"

const MIN_PASSWORD_LENGTH = 8

type Step = "password" | "otp" | "done"

export default function PoliceActivateWizard() {
  const router = useRouter()
  const [session, setSession] = useState<PoliceActivationSession | null>(null)
  ...
  useEffect(() => {
    const stored = loadPoliceActivationSession()
    setSession(stored)
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const result = await setPoliceActivationPassword(
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await verifyPoliceActivationOtp(session.challengeToken, otp.trim())
      clearPoliceActivationSession()
      setStep("done")
      setTimeout(() => {
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#080808] px-4 text-center">
        ...
        <p className="mt-2 max-w-md text-sm text-[#888]">
          Sign in with the temporary password from your administrator to start
          activation again.
        </p>
        <button
          type="button"
        ...
        {step === "otp" && (
          <form onSubmit={handleOtpSubmit} className="space-y-4">
            <p className="text-sm text-[#888]">
              ...
              <span className="text-[#ccc]">{session.email}</span>.
            </p>
            <div>
              <label className="mb-1 block text-[10px] uppercase tracking-wider text-[#666]">
                Email OTP
